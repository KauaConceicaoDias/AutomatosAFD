...
class AFD:
    ...
    def completar(self):
        """
        Completa o AFD adicionando um estado morto para transições indefinidas.
        Retorna um novo objeto AFD completo (não modifica o original).
        O estado morto é não-final e transita para si mesmo em todos os símbolos.
        """
        # Copia os componentes existentes
        novos_estados = self.estados[:]
        novas_transicoes = self.transicoes.copy()
        novos_finais = self.estados_finais[:]
        novo_inicial = self.estado_inicial
        novo_alfabeto = self.alfabeto[:]

        # Define nome do estado morto (evita conflitos)
        estado_morto = 'q_dead'
        contador = 1
        while estado_morto in novos_estados:
            estado_morto = f'q_dead{contador}'
            contador += 1

        # Adiciona o estado morto
        novos_estados.append(estado_morto)

        # Transições do estado morto para si mesmo em todo o alfabeto
        for simbolo in novo_alfabeto:
            novas_transicoes[(estado_morto, simbolo)] = estado_morto

        # Para cada estado existente, adiciona transições faltantes para o estado morto
        for estado in self.estados:  # Apenas os originais
            for simbolo in novo_alfabeto:
                if (estado, simbolo) not in novas_transicoes:
                    novas_transicoes[(estado, simbolo)] = estado_morto

        # Cria e retorna o novo AFD completo
        return AFD(
    ...
    def minimizar(self):
        # Retornar um novo AFD minimizado
        
        afd_completo = self.completar()
        
        if not afd_completo.estados:
            return afd_completo  # AFD vazio

        # Passo 1: Partição inicial: estados finais vs não-finais
        P = []  # Lista de conjuntos (partições)
        finais_set = set(afd_completo.estados_finais)
        nao_finais = set(afd_completo.estados) - finais_set
        ...
        # Fila de trabalho (W): partições a processar
        W = []
        if finais_set:
            W.append(finais_set)  # Começamos com os finais (convenção comum)

        # Mapeamento reverso: estado → partição atual (índice em P)
        estado_para_particao = {}
        for idx, part in enumerate(P):
            for q in part:
                estado_para_particao[q] = idx

        # Função auxiliar: inversa da transição (para cada símbolo, quem chega em um estado)
        # delta_inv[símbolo][estado_destino] = conjunto de estados que vão para ele com esse símbolo
        delta_inv = {simb: {q: set() for q in afd_completo.estados} for simb in afd_completo.alfabeto}
        for (q, a), p in afd_completo.transicoes.items():
            delta_inv[a][p].add(q)

        # Algoritmo principal de Hopcroft
        while W:
            A = W.pop(0)  # Pega uma partição da fila

            for c in afd_completo.alfabeto:  # Para cada símbolo do alfabeto
                X = set()  # Estados que têm transição por 'c' para algum estado em A
                for q in A:  # Correção: Iterar sobre A para encontrar predecessores
                    X.update(delta_inv[c][q])

                # Agora dividimos as partições que intersectam X
                novas_particoes = []
                for Y in P[:]:  # Copia para evitar modificação durante iteração
                    Y_inter_X = Y & X
                    Y_diff_X = Y - X

                    if not Y_inter_X or not Y_diff_X:
                        # Não divide
                        novas_particoes.append(Y)
                        continue

                    # Divide Y em duas partes
                    novas_particoes.append(Y_inter_X)
                    novas_particoes.append(Y_diff_X)

                    # Atualiza W: Se Y estava em W, substitui pelas novas
                    if Y in W:
                        W.remove(Y)
                        W.append(Y_inter_X)
                        W.append(Y_diff_X)
                        ...
                        # Adiciona a menor
                        if len(Y_inter_X) <= len(Y_diff_X):
                            W.append(Y_inter_X)
                        else:
                            W.append(Y_diff_X)

                    # Atualiza mapeamento
                    for part_idx, part in enumerate(novas_particoes):
                        for q in part:
                            estado_para_particao[q] = part_idx

                P = novas_particoes

        # Construção do AFD minimizado (resto igual, usando afd_completo)
        mapa_novos_estados = {}
        novos_estados = []
        novos_finais = []
        novas_transicoes = {}
        estado_inicial_novo = None

        for idx, part in enumerate(P):
            nome_novo = f"S{idx}"
            mapa_novos_estados[frozenset(part)] = nome_novo
            novos_estados.append(nome_novo)

            if afd_completo.estado_inicial in part:
                estado_inicial_novo = nome_novo

            if part & finais_set:
                novos_finais.append(nome_novo)

        for part in P:
            q_rep = next(iter(part))  # Um representante qualquer
            nome_origem = mapa_novos_estados[frozenset(part)]

            for a in afd_completo.alfabeto:
                if (q_rep, a) in afd_completo.transicoes:
                    destino = afd_completo.transicoes[(q_rep, a)]
                    for d_part in P:
                        if destino in d_part:
                            nome_destino = mapa_novos_estados[frozenset(d_part)]
                            novas_transicoes[(nome_origem, a)] = nome_destino
                            break

        afd_min = AFD(
            alfabeto=afd_completo.alfabeto,
            estados=novos_estados,
            transicoes=novas_transicoes,
            estado_inicial=estado_inicial_novo,
            estados_finais=novos_finais
        )
...
def main():
    ...
    while True:
        ...
        if opcao == "1":
            ...
            if automato_atual is None:
                print("\n[ERRO] Nenhum AFD definido!")
            elif not isinstance(automato_atual, AFD):
                print("\n[ERRO] Minimização só disponível para AFD (não AFND).")
            else:
                imprimir_cabecalho("MINIMIZAÇÃO DO AFD")
                print("Minimizando o AFD atual...")
                afd_minimo = automato_atual.minimizar()
                automato_atual = afd_minimo  # Atualiza o autômato atual
                print("\nAFD minimizado:")
                print(afd_minimo)
                print(f"Estados reduzidos para: {len(afd_minimo.estados)}")
                print("Transições:", afd_minimo.transicoes)
